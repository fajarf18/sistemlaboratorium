            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    <form action="{{ route('user.keranjang.store') }}" method="POST">
                        ...
                        @php
                            $stokPinjam = $barang->stok_pinjam;
                        @endphp
                        <td class="px-6 py-4 align-middle">{{ $stokPinjam }} pcs</td>
                        <td class="px-6 py-4 align-middle">
                            <div class="flex items-center justify-center space-x-2">
                                <button type="button" onclick="this.nextElementSibling.stepDown()" class="px-2 py-1 border border-gray-300 rounded-md text-gray-600 hover:bg-gray-100" @if($stokPinjam == 0) disabled @endif>-</button>
                                <input type="number" name="jumlah" value="1" min="1" max="{{ $stokPinjam }}" class="w-16 text-center border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" @if($stokPinjam == 0) disabled @endif>
                                <button type="button" onclick="this.previousElementSibling.stepUp()" class="px-2 py-1 border border-gray-300 rounded-md text-gray-600 hover:bg-gray-100" @if($stokPinjam == 0) disabled @endif>+</button>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center align-middle">
                            <button type="submit" 
                                    @if($stokPinjam == 0) disabled @endif
                                    class="px-4 py-2 text-white rounded-lg transition {{ $stokPinjam == 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700' }}">
                                + Tambah
                            </button>
                        </td>
                    </form>
